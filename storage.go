...
func SaveStudents(className string, students []StudentInput) error {
	if !isValidClassName(className) {
		return fmt.Errorf("invalid class/term name '%s'", className)
	}
	classTermPath := getClassTermPath(className)
...
func LoadStudents(className string) ([]StudentInput, error) {
	if !isValidClassName(className) {
		return nil, fmt.Errorf("invalid class/term name '%s'", className)
	}
...
func SaveAssignmentResults(className string, assignmentID string, results AssignmentOutput) error {
	if !isValidClassName(className) {
		return fmt.Errorf("invalid class/term name '%s'", className)
	}
...
func LoadAssignmentResult(className string, assignmentID string) (AssignmentOutput, error) {
	var results AssignmentOutput
	if !isValidClassName(className) {
